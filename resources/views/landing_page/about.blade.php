...
@push('styles')
    ...
    <style>
        ...
        .faq-button:hover .faq-text { color: var(--pink-primary); }
            ...
            <div class="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
                <div class="w-full lg:w-1/2 order-2 lg:order-1 animate-fadeIn">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <a href="#faq" class="px-8 py-4 bg-[#FF8FAB] text-white rounded-xl hover:bg-[#F472B6] transition-all duration-300 font-bold text-sm shadow-lg shadow-pink-100 flex items-center gap-2 no-underline">LIHAT FAQ</a>
                        <a href="https://wa.me/6285847255010" target="_blank" class="px-8 py-4 bg-white border border-gray-200 text-gray-700 rounded-xl hover:border-[#FF8FAB] hover:text-[#FF8FAB] transition-all duration-300 font-bold text-sm no-underline flex items-center gap-2">CHAT WHATSAPP</a>
                    </div>
                </div>

                <div class="w-full lg:w-5/12 order-1 lg:order-2 relative">
                    <div class="relative z-10 animate-fadeIn">
                        <img src="https://images.unsplash.com/photo-1516975080664-ed2fc6a32937?auto=format&fit=crop&q=80&w=600" class="w-full h-[400px] lg:h-[480px] object-cover rounded-2xl shadow-2xl border-8 border-white" />
                        <div class="absolute -bottom-6 -left-6 bg-white p-5 rounded-xl shadow-xl hidden md:block border border-gray-50">
                            ...
                            <p class="font-bold text-gray-900 text-base mb-0">Online Now</p>
        ...
        <div class="max-w-6xl mx-auto px-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-16">
                <div class="relative animate-fadeIn">
                    <img src="https://images.unsplash.com/photo-1604654894610-df63bc536371?auto=format&fit=crop&q=80&w=800" class="w-full rounded-2xl object-cover h-[500px] shadow-2xl" />
                ...
                <div class="lg:pt-0 pt-8 animate-fadeIn" style="animation-delay: 0.2s">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <img src="https://images.unsplash.com/photo-1560750588-73207b1ef5b8?auto=format&fit=crop&q=80&w=400" class="rounded-xl shadow-md h-40 w-full object-cover hover:scale-105 transition-transform duration-300">
                        <img src="https://images.unsplash.com/photo-1626379501846-0df4067b8bb9?auto=format&fit=crop&q=80&w=400" class="rounded-xl shadow-md h-40 w-full object-cover hover:scale-105 transition-transform duration-300">
                        <img src="https://images.unsplash.com/photo-1643968704781-df3b260df6a7?auto=format&fit=crop&q=80&w=400" class="rounded-xl shadow-md h-40 w-full object-cover hover:scale-105 transition-transform duration-300">
                        <img src="https://images.unsplash.com/photo-1522337660859-02fbefca4702?auto=format&fit=crop&q=80&w=400" class="rounded-xl shadow-md h-40 w-full object-cover hover:scale-105 transition-transform duration-300">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-6xl mx-auto px-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
                <div>
                    ...
                    <h3 class="mb-8 text-3xl md:text-4xl font-bold text-gray-900 font-serif-title">Sering Ditanyakan</h3>
                    <div class="space-y-4">
                        @forelse($faqs as $faq)
                        <div class="border-b border-gray-100 pb-4">
                            <button onclick="toggleFaq({{ $faq->id }})" class="faq-button w-full flex items-center justify-between text-left text-sm font-semibold text-gray-800 transition-colors py-2 focus:outline-none">
                                <span class="faq-text transition-colors duration-300">{{ $faq->question }}</span>
                                <svg id="icon-{{ $faq->id }}" class="w-4 h-4 text-gray-400 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                            </button>
                            <div id="answer-{{ $faq->id }}" class="faq-content">
                                <p class="text-sm text-gray-500 mt-2 leading-relaxed">{{ $faq->answer }}</p>
                            </div>
                        </div>
                        @empty
                        <p class="text-gray-400 italic">Belum ada pertanyaan yang tersedia.</p>
                        @endforelse
                    </div>
                </div>

                {{-- FORM HUBUNGI SUPPORT --}}
                <div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-2xl shadow-gray-100/50 animate-fadeIn">
                    ...
                    {{-- Notifikasi Sukses --}}
                    @if(session('success'))
                        <div class="mb-6 p-4 bg-green-50 border border-green-200 text-green-600 rounded-xl text-sm font-medium animate-fadeIn">
                            {{ session('success') }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('contact.send') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" required value="{{ old('name') }}" placeholder="Contoh: Sarah Johnson" 
                                   class="w-full border-b border-gray-100 pb-2 text-sm focus:outline-none focus:border-[#FF8FAB] transition-colors bg-transparent @error('name') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <textarea name="message" rows="2" required placeholder="Tuliskan pertanyaan Anda di sini..." 
                                      class="w-full border-b border-gray-100 pb-2 text-sm focus:outline-none focus:border-[#FF8FAB] transition-colors bg-transparent resize-none @error('message') border-red-500 @enderror">{{ old('message') }}</textarea>
