...
<div class="product-form-container">
    ...
    <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" class="form-style">
        ...
        <div class="form-group">
            ...
            <input type="text" name="namaproduct" id="namaproduct" value="{{ old('namaproduct') }}" placeholder="Contoh: PINK PERFECTION" required>
            @error('namaproduct') <div class="text-error">{{ $message }}</div> @enderror 
        </div>
        
        <div class="form-group">
            <label for="category">Kategori <span class="required">*</span></label>
            <input type="text" name="category" id="category" value="{{ old('category') }}" placeholder="Contoh: nail kit" required>
            @error('category') <div class="text-error">{{ $message }}</div> @enderror
        </div>
        
        <div class="form-group">
            <label for="stock">Stock Awal <span class="required">*</span></label>
            <input type="number" name="stock" id="stock" min="0" value="{{ old('stock', 0) }}" placeholder="Contoh: 10" required>
            @error('stock') <div class="text-error">{{ $message }}</div> @enderror
        </div>
        
        <div class="form-group">
            <label for="price">Harga (Rp) <span class="required">*</span></label>
            <input type="number" name="price" id="price" min="0" value="{{ old('price') }}" placeholder="Contoh: 50000" required>
            @error('price') <div class="text-error">{{ $message }}</div> @enderror
        </div>
        
        <div class="form-group">
            <label for="discount">Diskon (%)</label>
            <input type="number" name="discount" id="discount" min="0" max="100" value="{{ old('discount', 0) }}" placeholder="Contoh: 10">
        ...
        <div class="form-group">
            ...
            <small class="help-text">Default: Draft (tidak akan ditampilkan di website)</small>
        ...
        <div class="form-group">
            ...
            <small class="help-text">Format: JPG, PNG, GIF (Max 2MB)</small>
