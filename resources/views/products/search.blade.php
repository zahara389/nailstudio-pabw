...
@section('title', 'Hasil Pencarian: ' . $searchQuery . ' | Nail Studio')
...
@section('content')
    <section class="mx-auto max-w-7xl px-4 py-16">
        <div class="mb-12 text-center">
            <p class="text-sm font-semibold uppercase tracking-widest text-pink-500">Hasil Pencarian</p>
            <h1 class="mt-3 text-3xl font-bold text-gray-900 md:text-4xl">
                "{{ $searchQuery }}"
            </h1>
            <p class="mt-4 text-base text-gray-600 md:text-lg">
                @if ($resultCount > 0)
                    Ditemukan {{ $resultCount }} produk yang sesuai dengan pencarian Anda
                @else
                    ...
                    >
                        ...
                        <div class="flex flex-1 flex-col gap-4 p-5">
                            ...
                            <div class="mt-auto flex items-center gap-2">
                                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-medium {{ $product->status_classes }}">
                                    {{ $product->status_label }}
                                </span>
                                <span class="text-xs text-gray-500">{{ $product->stock_label }}</span>
                            </div>
                        </div>
                    </a>
                @endforeach
        ...
        @else
            <div class="col-span-full flex flex-col items-center rounded-3xl bg-white/80 p-12 shadow-lg shadow-pink-100">
                ...
                <h3 class="mt-6 text-xl font-semibold text-gray-900">Produk tidak ditemukan</h3>
                <p class="mt-3 text-center text-sm text-gray-600">
                    Maaf, kami tidak menemukan produk yang sesuai dengan pencarian "{{ $searchQuery }}".
                    <br>
                    Coba gunakan kata kunci yang berbeda atau jelajahi kategori kami.
                </p>
                <div class="mt-6 flex gap-3">
                    <a href="{{ route('products.index') }}" class="rounded-full bg-pink-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-pink-600">
                        Lihat Semua Produk
                    </a>
                    <a href="{{ route('landing.index') }}" class="rounded-full border-2 border-pink-500 px-6 py-3 text-sm font-medium text-pink-500 transition hover:bg-pink-50">
                        Kembali ke Beranda
                    </a>
                </div>
            </div>
